...
def handle_rockblock():
    ...
    try:
        ...
        full_data = {
            ...
            "lat": float(message_data.get("latitude", 0.0)),
            "lon": float(message_data.get("longitude", 0.0)),
            "alt": message_data.get("altitude", 0),
            "pres": message_data.get("pressure_mbar", 0),
            "temperature_pht_c": message_data.get("temperature_pht_c", 0),
            "temp_cj_c": message_data.get("temperature_cj_c", 0),
...
def live_data():
    data = {
        "latitude": message_history[-1]["lat"] if message_history else "No data",
        "longitude": message_history[-1]["lon"] if message_history else "No data",
        "timestamps": message_history[-1]["sent_time"] if message_history else "No data",
        "altitudes": message_history[-1]["alt"] if message_history else "No data"
    }
    return jsonify(data)
